...
// const PLACEHOLDERS = [
//   "Hello Luigi, how can I help you?",
//   "Ask me anything",
...
const AIChatInput = ({ onSend }) => {
  ...
  const sendMessage = async () => {
    if (!inputValue.trim()) return;
    if (onSend) {
      await onSend(inputValue, attachment);
    }
    setInputValue("");
    setAttachment(null);
  };
    ...
    <div className="w-full flex justify-center items-center text-black">
      ...
      >
        <div className="flex flex-col items-stretch w-full h-full">
          ...
          {/* Attachment Preview */}
          {attachment && (
            <div className="mt-2 px-4 flex items-center max-w-md">
              <div className="flex items-center bg-gray-100 rounded-full px-3 py-1 text-sm text-gray-700 w-full max-w-full truncate">
